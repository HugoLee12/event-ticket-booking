...
- **GĐ4-B (báo cáo) ĐÃ XONG toàn bộ 7 mục của B**: viết trong `docs/report/` mục 1 Giới thiệu (vì sao phân tán/SOA thay vì monolith), mục 2 Kiến trúc (Ch.17/18, kèm sơ đồ kiến trúc + sequence composition), mục 3 Concurrency & Resilience (Ch.14/17, kèm sơ đồ race), mục 5 Reuse & trade-off (Ch.15/16), mục 6 CI/CD (Ch.25), mục 7 Kiểm thử & đo lường (Ch.14 observability), mục 9 Kết luận & bài học; tất cả nhúng số harvest thật (race 1×201/19×409/COUNT=1, 13 test 4.919s, `/metrics` JSON, pino log 3 trạng thái). Mục 1 + 9 vốn PLAN bỏ trống, B nhận làm (đã cập nhật PLAN mục 7).
- **Bảo mật (rà chéo GĐ4)**: B phát hiện + vá lỗ hổng leo thang đặc quyền trong `services/auth/src/routes.ts` (register nhận `role` từ client → tự phong admin); đã bỏ `role` khỏi registerSchema, ép `'user'`, thêm test hồi quy trong `auth.test.ts` (8/8 auth test pass, CI xanh). Dòng STRIDE `/metrics` trong `04-bao-mat.md` từng ghi sai "Middleware requireAdmin" - đã sửa thành kiểm tra inline trong `services/booking/src/metrics.ts` (commit `feac4f6`).
- **Pass review chốt cuối (spec GitHub Issue [#1](https://github.com/HugoLee12/event-ticket-booking/issues/1)) - ĐÃ XONG TOÀN BỘ (code + báo cáo + spec, đã commit)**: 5 sub-agent song song xong (bug/đúng đắn, bảo mật, spec+contract, fidelity báo cáo, cấu trúc báo cáo) → gộp 1 danh sách 1 CRITICAL + 2 HIGH + 11 MEDIUM + ~20 LOW, user đã duyệt "toàn bộ".
Đã fix + verify (build sạch, SQL Server thật, booking 17 test + auth 10 test pass) các mục CRITICAL/HIGH/MEDIUM thuộc code: race `CREATE DATABASE` lúc boot compose trên volume sạch (bắt lỗi 1801, cả 2 `init.ts`) + `restart: on-failure` (docker-compose.yml); Booking thêm `helmet`; JSON body hỏng cú pháp → 400 thay vì 500 (cả 2 `app.ts`); `seatId` vượt INT → 400 (`bookings.ts` `.max(2147483647)`); `jwt.verify` pin `algorithms: ['HS256']` (cả 2); Auth register trùng email đồng thời → 409 thay vì 500 (bắt lỗi 2627/2601, `routes.ts`); `loginLimiter` thêm `skipSuccessfulRequests: true`.
Đã commit tách nhãn rõ: `3e3dbe2` fix(booking) phần B + file chung, `da238db` fix(auth) [cho A review] phần A, `e717e12` docs sửa báo cáo + spec.
Phần câu chữ đã sửa đủ theo danh sách duyệt: CRITICAL `08-dao-duc.md` fallback đổi thành `user: null`; mục 03 (ConsecutiveBreaker, message sơ đồ, lệnh test), mục 04/08 (heading `#`, văn xuôi, cost factor 10, bỏ Buffer Overflow, role check inline, helmet 2 service, rate-limit chỉ tính login sai), mục 01 (nêu đủ 7 chương), 07 (lệnh test), 09 (thống nhất "optimistic locking"); spec thêm 404 POST /bookings, bỏ FK `Booking.user_id` (ghi rõ cố ý), thêm `display_name` vào User.
Ghi nhận để lại tuần ghép cuối: tổng độ dài báo cáo ước ~21-26 trang, vượt mốc 12-15 trang trong PLAN.md - xử lý khi ghép.
- **GĐ4-B build báo cáo (issue [#2](https://github.com/HugoLee12/event-ticket-booking/issues/2)) đã xong + verify + push `main` (`ea5ebfb`)**: `docs/report/build_report.py` sinh `report-body.docx` (TOC + thân 9 mục) từ 9 file `0X-*.md`, có self-check assert-based. Sơ đồ mermaid render TỰ ĐỘNG lúc build: khối ` ```mermaid ` trong `.md` là nguồn duy nhất, script POST text tới `kroki.io/mermaid/png` rồi cache PNG theo sha1 nội dung tại `docs/report/img/mermaid-<hash8>.png` (cache hit chèn inline không gọi mạng; đổi sơ đồ → đổi hash → tự render lại; mất mạng và chưa cache → placeholder, build vẫn ra docx). Đã bỏ cách cũ (render tay qua mermaid.live + `![](img/tên.png)` tên cố định). Chỉ cần `pip install python-docx` (còn lại stdlib `urllib`), KHÔNG cài mermaid-cli/pandoc/Chromium; đánh đổi: cần mạng lần đầu mỗi khi sơ đồ đổi, bù bằng cache đã commit. Verify: build ra 3 PNG thật + docx 3 inline image (0 placeholder), self-check OK, chạy lại là cache hit không gọi mạng.
- Việc kế GĐ4: A còn mục 4 Bảo mật + bảng STRIDE + mục 8 Đạo đức (bản nháp đã có trong `docs/report/04-bao-mat.md` + `08-dao-duc.md`, đã tách theo quy ước đánh số). Chung: ghép báo cáo tuần cuối + slide + demo end-to-end. A nhớ `git pull --rebase` để lấy code auth đã vá + E2E test.
