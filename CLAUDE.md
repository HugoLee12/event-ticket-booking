...
- **Bảo mật (rà chéo GĐ4)**: B phát hiện + vá lỗ hổng leo thang đặc quyền trong `services/auth/src/routes.ts` (register nhận `role` từ client → tự phong admin); đã bỏ `role` khỏi registerSchema, ép `'user'`, thêm test hồi quy trong `auth.test.ts` (8/8 auth test pass, CI xanh). A còn 1 chỗ trong `04-bao-mat.md`: dòng STRIDE `/metrics` ghi "Middleware requireAdmin" nhưng thực tế là kiểm tra inline trong `services/booking/src/metrics.ts`.
